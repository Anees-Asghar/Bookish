...
class User_Book(db.Model):
    ...
    book_id = db.Column(db.String(20), db.ForeignKey(
        Book.book_id), primary_key=True, nullable=False)
    status = db.Column(db.Enum(status), nullable=False)
    rating = db.Column(db.Float, db.CheckConstraint(
        "rating >= 0 AND rating <= 5"), nullable=True)
