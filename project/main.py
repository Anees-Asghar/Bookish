from flask import Flask, Blueprint, redirect, request, render_template, url_for, send_from_directory, flash
...
def index():
	...
	reading_nows = db.session.query(User_Book, Book).filter(User_Book.user_id==user_id, User_Book.status==status.reading_now).filter(User_Book.book_id==Book.book_id).order_by(desc(User_Book.last_modified)).all()
	have_reads = db.session.query(User_Book, Book).filter(User_Book.user_id==user_id, User_Book.status==status.have_read).filter(User_Book.book_id==Book.book_id).order_by(desc(User_Book.last_modified)).all()
	to_reads = db.session.query(User_Book, Book).filter(User_Book.user_id==user_id, User_Book.status==status.to_read).filter(User_Book.book_id==Book.book_id).order_by(desc(User_Book.last_modified)).all()
...
def detail(book_id):
	...
	query = User_Book.query.filter_by(book_id=book_id, user_id=current_user.user_id,).first()
	rel = None
	if query:
	...
	reviews = db.session.query(User_Book, User).filter(User_Book.book_id==book_id, User_Book.user_id!=current_user.user_id).filter(User_Book.user_id==User.user_id).order_by(desc(User_Book.last_modified)).all()
	
	# Query database for average rating for the book
	avg_rating = User_Book.query.with_entities(func.avg(User_Book.rating).label('rating')).filter(User_Book.book_id==book_id).first()
